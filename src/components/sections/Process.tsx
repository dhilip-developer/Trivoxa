...
export default function Process() {
  ...
  return (
    <section id="process" className="py-24 bg-transparent relative overflow-hidden" ref={processRef}>
        ...
        <div className="rounded-3xl bg-black/40 backdrop-blur-md border border-white/10 p-8 md:p-12 shadow-2xl">
          ...
          <div className="md:hidden max-w-4xl mx-auto space-y-4">
            {steps.map((step, index) => (
              <div
                key={index}
                className="bg-black/50 backdrop-blur-sm border border-white/10 rounded-lg overflow-hidden transition-all duration-300 hover:border-orange/30"
              >
                <button
          ...
          <div className="hidden md:block relative">
            ...
            {steps.map((step, index) => (
              <div
                key={index}
                className={`process-item mb-16 lg:mb-32 transform transition-all duration-700 ${index % 2 === 0 ? 'lg:pr-[50%]' : 'lg:pl-[50%] lg:text-right'
                  } ${visibleSteps[index] ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-20'}`}
              >
                <div className="relative">
                  {/* Step number with glowing circle */}
                  <div className={`inline-flex items-center justify-center w-14 h-14 rounded-full bg-black border border-orange mb-4 relative z-20 ${index % 2 === 0 ? 'lg:float-right lg:ml-8 lg:-mr-7' : 'lg:float-left lg:mr-8 lg:-ml-7'
                    }`}>
                    <div className="absolute inset-0 rounded-full bg-orange/10 blur-[5px]"></div>
                    <span className="text-orange-light font-bold relative z-10">{step.number}</span>
                  </div>
                  ...
                  {/* Content */}
                  <div className={`bg-black/50 backdrop-blur-sm border border-white/10 p-6 rounded-lg hover:border-orange/30 transition-all ${index % 2 === 0 ? 'lg:mr-12' : 'lg:ml-12'
                    }`}>
