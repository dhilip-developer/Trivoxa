...
const Card = ({ children, className }) => <div className={`p-6 border rounded-lg ${className}`}>{children}</div>;
const CardHeader = ({ children }) => <div className="mb-4">{children}</div>;
const CardTitle = ({ children, className }) => <h3 className={`font-semibold text-lg ${className}`}>{children}</h3>;
...
export default function Services() {
  ...
  return (
    <section id="services" className="py-24 bg-gradient-to-b from-background to-tech-dark">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {services.map((service, index) => (
            <Card key={index} className="bg-tech-dark/50 backdrop-blur-sm border-gray-800 hover:border-orange/50 transition-all hover:shadow-lg hover:shadow-orange/10 hover:-translate-y-1 overflow-hidden group">
              <CardHeader>
                <div className="w-12 h-12 rounded-lg bg-orange/10 flex items-center justify-center text-orange-light mb-4 group-hover:bg-orange/20 transition-colors">
                  {service.icon}
                </div>
                <CardTitle className="text-xl text-orange-light">
                  {service.title}
                </CardTitle>
              ...
              <CardContent>
                <div className="flex flex-wrap gap-2 mt-2">
                  {service.features.map((feature, i) => (
                    <div key={i} className="px-2.5 py-0.5 text-xs bg-black/30 border border-gray-800 rounded-full text-gray-300">
                      {feature}
                    </div>
                  ))}
                </div>
